...
export default class Carrito {
  constructor() {
    this.carritoLOG = 'public/log/carrito.txt';
    this.productos = null;
    this.CARRITO = {};

    (async () => {
      this.CARRITO = await this.getInitialCarrito();
    })();
  }
  ...
  async getInitialCarrito() {
    try {
      if (fs.existsSync(this.carritoLOG)) {
        const initialCarrito = JSON.parse(await fs.promises.readFile(this.carritoLOG, 'utf8'));
        return {
          id: initialCarrito.id,
          timestamp: new Date(initialCarrito.timestamp),
          productos: initialCarrito.productos
        }
      } else {
        return {
          id: 0,
  ...
  async saveFile(carrito) {
    try {
      await fs.promises.writeFile(this.carritoLOG, JSON.stringify(carrito, null, "\t"));
    } catch(err) {
      console.log(err);
    }
  }
